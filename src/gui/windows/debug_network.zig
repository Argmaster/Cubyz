...
pub fn render() void {
	...
	if (main.game.world != null) {
		...
		y += 8;
		inline for (@typeInfo(network.protocols.Protocols).@"enum".fields) |field| {
			draw.print("{s}: received {}kiB sent {}kiB", .{field.name, network.protocols.bytesReceived[field.value].load(.monotonic) >> 10, network.protocols.bytesSent[field.value].load(.monotonic) >> 10}, 0, y, 8);
			y += 8;
		}
	}
